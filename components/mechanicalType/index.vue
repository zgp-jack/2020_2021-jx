        ...
        <div class="mechanics-inner">
            <div class="top">
                ...
                <div class="mechanics">
                    <div v-if="selectData.length">
                        <span v-for="(item,index) in selectData" :key="index" @click="colorSelect(item)">{{item.name}}<i class="itemclsoe"/></span>
                    </div>
                </div>
            </div>
            <div class="mechanics_select" ref="select_box">
                ...
                <div class="list" v-for="(item,index) in clas" :key="index">
                    <div>
                        <p class="list_title clearfix"  :class="[index == clas.length - 1 && 'nonedet']" @click="(e)=>showSelect(index,e)">{{item[0].name}}<i :class="['jiantou','',!item[0].show?'rotate':'']"/><span v-if="item[0].number">{{item[0].number}}</span></p>
